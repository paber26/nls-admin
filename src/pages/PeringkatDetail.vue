    ...
    <main class="flex-1">
      ...
      <div class="px-6 py-8">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-md font-semibold">Leaderboard - {{ tryoutTitle }}</h2>
          <RouterLink to="/peringkat" class="text-sm px-4 py-2 bg-slate-200 rounded-lg hover:bg-slate-300">
            Kembali
          </RouterLink>
        </div>
            ...
            <table class="w-full text-sm">
              ...
              <tbody>
                <tr v-for="item in ranking" :key="item.user_id" class="border-t hover:bg-slate-50">
                  ...
                  <td class="px-4 py-3 text-center font-bold text-indigo-600">{{ item.nilai }}</td>
                </tr>
              </tbody>
            </table>
          </div>
