  ...
  <AppShell>
    <template #header>
      <div class="flex w-full flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-lg font-semibold text-slate-800">Tambah Soal Native CP</h1>
          <p class="text-sm text-slate-500">Buat soal baru beserta informasi Time/Memory Limit</p>
        </div>
        <div class="flex gap-3">
          <RouterLink to="/cp-problems" class="flex items-center gap-2 px-4 py-2 rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-indigo-600 transition font-medium text-sm bg-white">
      ...
      <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div class="xl:col-span-2 space-y-6">
          ...
          <div class="flex justify-end gap-3">
            ...
            <button @click="submit" :disabled="loading" class="px-5 py-2.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition font-medium flex items-center justify-center min-w-[120px]">
...
const submit = async () => {
  ...
  loading.value = true
  try {
    await api.post('/cp-problems', form.value)
